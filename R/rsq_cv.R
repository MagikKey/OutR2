...
rsq_cv <- function(data, glmfit, k = 10, balanced = F, J = 2){
  ...
  if(k == n){
    ...
    for (i in 1:k){
      ...
      fit_k <- glm(formula = as.formula(glmfit$formula), data = train_set_k)
      ...
      if(! is.finite(R2_k[i]) ) R2_k[i] <- NA # Check if the value is finite
  ...
  RMSE_Cv <- sqrt(MSE_Cv) # Cross-validated Root of the Mean Squared Error
